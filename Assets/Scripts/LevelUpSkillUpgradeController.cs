...
public class LevelUpSkillUpgradeController : MonoBehaviour
{
    ...
    private void ApplyUpgrade(LevelUpCardData card)
    {
        ...
        switch (card.Effect)
        {
            ...
            case LevelUpCardEffect.FIRERING:
                FireRingController fireRing = GetPlayerFireRing();
                if (fireRing != null)
                {
                    fireRing.ApplyLevelUpCardValue(value);
                }

                break;
            case LevelUpCardEffect.EXPLOSION:
                ExplosionController explosion = GetPlayerExplosion();
                if (explosion != null)
                {
                    explosion.ApplyLevelUpCardValue(value);
                }
    ...
    private FireRingController GetPlayerFireRing()
    {
        if (playerFireRing != null)
        {
    ...
    private ExplosionController GetPlayerExplosion()
    {
        ...
        GameObject player = GameObject.FindGameObjectWithTag(PlayerTag);
        playerExplosion = player != null ? player.GetComponent<ExplosionController>() : null;
        return playerExplosion;
    }
}
